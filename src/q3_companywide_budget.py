...
from common import load_drug_data, load_covariance


def solve_q3(data_dir=None, variance_cap=None, minimize_variance=False, output_flag=0):
    df, projects = load_drug_data(data_dir)
    cov = load_covariance(data_dir)

    m = Model("zinca_q3_companywide")
    ...
    m.addConstr(
        quicksum(df.loc[df["Project"] == j, "Cost"].iloc[0] * x[j]
                 for j in projects) + U == 1000,
        name="company_budget_balance",
    )
    ...
    if variance_cap is not None:
        m.addQConstr(var_expr <= variance_cap)
...
def build_frontier(data_dir=None, n_points=20):
    sol = solve_q3(data_dir)
    caps = np.linspace(0, sol["variance"], n_points)
    sd, value = [], []
    for cap in caps:
        f = solve_q3(data_dir, variance_cap=cap)
        sd.append(f["stdev"])
        value.append(f["total_value"])
    return sd, value
...
if __name__ == "__main__":
    ...
    plt.title("Efficient Frontier (Q3: Company-Wide Budget)")
    plt.grid(True)
    plt.show()
